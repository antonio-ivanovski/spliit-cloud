...
export function RemovedParticipantBadge({ className }: { className?: string }) {
  ...
  return (
    <Badge
      variant="outline"
      className={`shrink-0 border-amber-500/35 bg-amber-500/10 px-1.5 py-0 text-[10px] font-medium tracking-wide text-amber-800 uppercase dark:text-amber-300 ${className ?? ''}`}
    >
      {t('removedBadge')}
    </Badge>
  )
