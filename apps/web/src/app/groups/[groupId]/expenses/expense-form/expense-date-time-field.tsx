...
export function buildExpenseTimeTimeline(
  ...
  const gapDays = new Map<string, boolean>()
  ...
  return [...candidateTimes]
    ...
    .filter((option) => {
      let hasGap = gapDays.get(option.dateIso)
      if (hasGap === undefined) {
        hasGap = hasWallTimeGap(option.dateIso, timeZone)
        gapDays.set(option.dateIso, hasGap)
      }
      return (
        !hasGap ||
        isValidWallTime(option.dateIso, parseTimeMinutes(option.time), timeZone)
      )
    })
}

function isoFromExpenseDate(value: Date | string | undefined): string {
  if (value instanceof Date && !Number.isNaN(value.getTime())) {
    return `${value.getUTCFullYear()}-${String(value.getUTCMonth() + 1).padStart(2, '0')}-${String(value.getUTCDate()).padStart(2, '0')}`
  }
  const parsed = value ? parseIsoCalendarDate(String(value).slice(0, 10)) : null
  return parsed
    ? toIsoCalendarDate(parsed)
...
export function ExpenseDateTimeField({ form, readOnly, sExpense }: Props) {
  ...
  const expenseDateRaw = useWatch({
    control: form.control,
    name: 'expenseDate' as never,
  }) as unknown as Date | string | undefined
  ...
  const selectedTz = expenseTimeZone ?? deviceTz
  const timeStr = expenseTime ?? ''
  const isoDate = useMemo(
    () => isoFromExpenseDate(expenseDateRaw),
    [expenseDateRaw],
  )
  ...
  const timeline = useMemo(
    () => buildExpenseTimeTimeline(timelineAnchor, selectedTz),
    [selectedTz, timelineAnchor],
  )
