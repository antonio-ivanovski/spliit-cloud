...
export const groupsRouter = createTRPCRouter({
  ...
  /** Get a single group plus the caller's membership and link-invite state. Read-accessible to pending link-invitees via `linkInviteToken`. */
  get: getGroupProcedure,

  /** Get a group with participants and whether it has expenses. Used by the import flow. */
  getDetails: getGroupDetailsProcedure,

  list: listGroupsProcedure,

  /** Create a new group with ledger and participants. The caller becomes the first ADMIN. */
  create: createGroupProcedure,

  /** Update a group's name, currency, information, or participants. */
  update: updateGroupProcedure,

  /** Archive or unarchive a group. Archived groups reject new expenses. Use `force` to archive even when balances are non-zero. */
  ...
  /**
   * Leave a group. Requires `force` if you're the last admin or have non-zero balances;
   * pass `promoteMemberId` to transfer admin first.
   */
  leave: leaveGroupProcedure,

  /** Preview the consequences of leaving: unsettled balances you'd forfeit and members eligible for admin promotion. */
  ...
  /**
   * Create or extend a group from parsed external data.
   * When `targetGroupId` is set, merges into an existing group;
   * otherwise creates a new one from `groupFormValues`.
   */
  import: importGroupProcedure,

  /** Fetch and parse a group from an external URL, returning a preview without persisting anything. */
  importPreview: previewFromUrlProcedure,

  /** Check whether a group was imported from an external source and return the source provider + URL. */
