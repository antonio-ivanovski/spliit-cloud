...
/**
 * Read-only summary the web client uses to render the admin "remove member"
 * dialog. Returns the target's display name and whether they have unsettled
 * balances so the dialog can decide between the simple confirm and the
 * three-option variant (settle+remove, remove only, cancel).
 */
...
/**
 * Remove an active member from a group. Ledger participants and historical
 * expenses are intentionally preserved (the membership row is flipped to
 * `REMOVED` and `leftAt` is set so the active roster hides them but balances
 * still resolve correctly).
 *
 * The caller cannot remove themselves — admins must use the dedicated "leave
 * group" flow. Removing the last admin is rejected so the group always keeps at
 ...
 * `settleBalances: true` (create settlement expenses for the legs involving the
 * target before flipping the membership) or `settleBalances: false` (remove
 * without touching the ledger). When the flag is missing the mutation throws
 * `PRECONDITION_FAILED` so the web client can re-render the remove dialog with
