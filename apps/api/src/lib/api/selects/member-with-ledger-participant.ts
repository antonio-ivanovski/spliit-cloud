 ...
 * `ledgerParticipant.id` (needed for participant-scoped lookups) and
 * optionally pulls the linked `account.name` (needed for permission and
 * notification flows that display the actor's name).
 *
 * Use `includeAccount: true` for member-detail queries (update, remove,
 * leave preview) and `includeAccount: false` for lightweight status
 * checks (revoke, decline).
 */
