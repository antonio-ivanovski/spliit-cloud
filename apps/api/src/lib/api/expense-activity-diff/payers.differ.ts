 ...
 *
 * Wraps {@link payerSemantics} to apply semantic comparison that avoids false
 * positives when `paidBySplitMode` is `BY_AMOUNT` (shares are derived from the
 * expense total, so an amount-only edit should not flag a payer change).
 */
