...
The Splitwise CSV parser always produces expenses in the row currency with `originalAmount`, `originalCurrency`, and `conversionRate` set to `null`. Cross-currency conversion SHALL be handled by `buildImportBatch` using the unified `share-calculation` core and the same server exchange resolver as native expense create (Frankfurter for fiat legs, Coinbase for crypto legs, with bridging). The import currency conversion step SHALL show provider attribution for automatic rates.
