...
- Close notification onboarding immediately when enabling push, keep setup running in the background, and reopen only if setup fails (`TBD` by @TBD)
...
- Fixed mobile search pickers staying behind the iOS software keyboard: currency, category, participant, timezone, and language drawers now fit their visible results, grow to a keyboard-aware maximum, and scroll longer lists within the visible viewport, with native Android keyboard resizing as a progressive enhancement — fixes #124 (`TBD` by @TBD)
